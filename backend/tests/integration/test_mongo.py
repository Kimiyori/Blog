...
async def insert_users(session_mock):
    await session_mock[0].test.users.insert_many([dict(user) for user in USERS], session=session_mock[1])


@pytest.mark.asyncio
async def test_users_can_load_lines(session_mock):
    await insert_users(session_mock)
    count = await session_mock[0].test.users.count_documents({},session=session_mock[1])
